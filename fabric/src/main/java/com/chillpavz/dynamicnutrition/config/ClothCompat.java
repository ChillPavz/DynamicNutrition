...
/**
 * Everything that touches cloth-config, on Fabric.
 *
...
public final class ClothCompat {
    ...
    // Fabric's cloth build has the mod id 'cloth-config' and PROVIDES 'cloth-config2'. The two are
    // checked because a provided id and a real id are not the same lookup, and the NeoForge build
    // uses a third spelling again ('cloth_config').
    private static final String[] IDS = {"cloth-config", "cloth-config2"};
